...
export const projectsDataFr: ProjectProps[] = [
  {
    ...
    shortDescription:
      'Application full-stack de gestion de bars à bières : gestion des bières, disponibilités et carte dynamique pour simplifier le quotidien des gérants.',
    detailedDescription:
      "Projet full-stack personnel visant à résoudre un problème concret rencontré par les gérants de bars à bières. L'application centralise la gestion des bières (CRUD complet), des formats, des disponibilités (stock disponible/non disponible) et génère automatiquement une carte interactive et claire pour les clients. Elle intègre une gestion multi-niveaux de comptes (utilisateur, admin, super-admin), une architecture modulaire scalable, et suit les meilleures pratiques de développement (typage strict, validation, sécurité). L'objectif est de fournir un outil professionnel, ergonomique et fiable qui répond à de vrais besoins métier.",
    stack: [
      {
        category: 'language',
        name: 'Languages',
    ...
    contributions: [
      "Conception de l'architecture globale (backend REST, frontend React, base de données)",
      'Modélisation complète de la base de données relationnelle (bières, formats, bars, utilisateurs, ...)',
      "Développement de l'interface de gestion complète : dashboards, filtres avancés, CRUD sur toutes les entités",
      'Implémentation de la carte interactive dynamique pour les clients (affichage en temps réel des bières disponibles)',
      'Système de gestion des disponibilités (stock disponible/non disponible) avec mise à jour en temps réel',
      'Création de formulaires robustes et strictement typés avec React Hook Form et Zod',
      'Tableaux de données structurés et filtrables avec TanStack Table pour les dashboards',
      "Développement de l'API REST complète : authentification, CRUD sécurisé, middlewares de validation",
      'Système de gestion de comptes multi-niveaux (user, admin, super-admin) avec permissions différenciées',
      'Interface utilisateur cohérente et accessible avec Shadcn/ui et Radix UI',
      'Configuration stricte ESLint/Prettier dans un monorepo pour garantir la qualité du code',
      'Mise en place de la sécurité : protection CSRF, rate limiting, middlewares de validation, protection DDoS (Cloudflare)',
      'Déploiement en production avec domaine personnalisé et configuration DNS (Cloudflare)',
      'Monitoring des erreurs et performances (Sentry) et surveillance de la disponibilité (UptimeRobot)',
      "Intégration d'analytics respectueux de la vie privée (Plausible Analytics)",
    ],
    context: {
