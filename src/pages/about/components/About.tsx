...
const About = ({ aboutDataList }: AboutProps) => {
  return (
    <section className="bg-bg-light border-border-light text-text-light mx-auto my-4 w-1/2 rounded-2xl border">
      ...
      {aboutDataList.map((item) => (
        <section key={item.id} className="px-8">
          ...
          {item.content.map((paragraph) => (
            <p key={paragraph.id} className="font-text-dark max-w-[80ch]">
